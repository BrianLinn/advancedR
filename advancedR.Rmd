...
Don't have a target value
Low number of cases needed - used in preliminary analysis 

###Clustering

* Grouping cases into clusters
    + objects in cluster have high similarity on attribute values
    + class labels not known
...
library(tidyverse)
TM <- read_csv("C:/MOVEDELETE/OneDrive-2018-11-05/AdvancedRCode/TM.csv")
TM <- filter(TM, TM$CustomerKey %% 400 == 0)
TM <- select(TM, TotalChildren, NumberChildrenAtHome, HouseOwnerFlag, NumberCarsOwned, BikeBuyer)
...
TMClust$Income <- TMClust$YearlyIncome
...
library("tidyverse")
library(arules)
df_AR <- read_csv("C:/MOVEDELETE/OneDrive-2018-11-05/AdvancedRCode/TM.csv")
df_AR <- df_AR[, c("CustomerKey", "Gender")]
# # Read the data
# library(RODBC)
# con <- odbcConnect("AWDW", uid = "RUser", pwd = "Pa$$w0rd")
...
# close(con)
# df_AR$OrderNumber = df_AR$CustomerKey
# df_AR$Model = df_AR$Gender
# View(df_AR)
# 
...
DM tool to capture complex input and output

Input variables defined as input units - combined into new internal variables (hidden layer or layers) then redirect to output

```{r}
con <- odbcDriverConnect('driver={SQL Server};
                                server=CeaEqaDvVmSql02.Calquake.com;
                                database=AdventureWorksDW2012;
                                trusted_connection=true')
con <- odbcConnect("ceaEqaDvVmSql02", uid="svc_bireporting", pwd="ezuQbE8L78TS3SAYgnUK")
TM <- as.data.frame(sqlQuery(con, 
...
Accurate results may rely on 

* de-trending
* filtering and smoothing (remove seasonality)
* standardization (normalization) of time points
...
# Prepare for reading
cname <- file.path("C:\\MOVEDELETE\\OneDrive-2018-11-05\\AdvancedRCode\\Corpus")
